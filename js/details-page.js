...
async function loadEvent(){
  showLoading(status,"Loading event details…");content.hidden=true;
  try{const event=await getEarthquake(id),properties=event.properties||{},coordinates=event.geometry?.coordinates;if(!Array.isArray(coordinates))throw new Error("This event has no coordinates");const [longitude,latitude,depth]=coordinates;
    ...
    const map=L.map("map").setView([latitude,longitude],6);L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:'© <a href="https://www.openstreetmap.org/" target="_blank" rel="noopener noreferrer">OpenStreetMap</a> contributors',maxZoom:18}).addTo(map);L.circleMarker([latitude,longitude],{radius:12,fillColor:"#ed5b36",color:"#182019",weight:2,fillOpacity:.9}).addTo(map).bindPopup(properties.place||"Earthquake epicenter").openPopup();
    status.hidden=true;content.hidden=false;setTimeout(()=>map.invalidateSize(),0);
