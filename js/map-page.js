...
function renderMap(){
  if(!map){map=L.map("map",{worldCopyJump:true}).setView([18,10],2);L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:'© <a href="https://www.openstreetmap.org/" target="_blank" rel="noopener noreferrer">OpenStreetMap</a> contributors',maxZoom:18}).addTo(map);markerLayer=L.layerGroup().addTo(map);}
  ...
  status.hidden=true; panel.hidden=false; setUpdateSummary(count,updated,events.length,generatedAt); setTimeout(()=>map.invalidateSize(),0);
}
